...
function AIDetection() {
    ...
    const [precision, setPrecision] = useState(currentProject.detectionResult.input.precision || 'yolov5s');
    ...
    const handleStartDetection = async () => {
        ...
        try {
            console.log(currentProject.detectionResult.input);
        ...
        <div className="fade-in">
            ...
            <div className="card" style={{ marginTop: '24px' }}>
                ...
                <div className="precision-selector" style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <Select
                        defaultValue={currentProject.detectionResult.input.precision || 'yolov5s'}
                    ...
                    <Row gutter={[16, 16]}>
                        <Col span={12}>
                            <div className="card" style={{ height: '100%', padding: '16px' }}>
                                ...
                                <p><strong>分析状态：</strong>{currentProject.detectionResult.output.isValid === 1 
                                    ? "✅ 有效图像 - 成功检测到损伤特征" 
                                    : currentProject.detectionResult.output.isValid === -1 
                                        ? "❌ 图像无效 - 未检测到明确损伤。请确保齿面平行于镜头，且画面中仅包含单个齿面，光照均匀无强反光" 
                                        : currentProject.detectionResult.output.isValid}
                                </p>
                                <p><strong>磨损状况评估：</strong>{parseFloat(currentProject.detectionResult.output.abrasionRate).toFixed(1)}% 
                                    {parseFloat(currentProject.detectionResult.output.abrasionRate) > 80 
                                        ? "（大面积损伤 - 请检查干涉情况）" 
                                        : parseFloat(currentProject.detectionResult.output.abrasionRate) > 60 
                                            ? "（中等损伤面）" 
                                            : "（轻微）"}
                                </p>
                                <p><strong>齿面剥落分析：</strong>{parseFloat(currentProject.detectionResult.output.peelingRate).toFixed(1)}% 
                                    {parseFloat(currentProject.detectionResult.output.peelingRate) > 30 
                                        ? "（大面积损伤 - 请检查干涉情况）" 
                                        : parseFloat(currentProject.detectionResult.output.peelingRate) > 10 
                                            ? "（警告）" 
                                            : "（可接受）"}
                                </p>
                                <p><strong>压伤程度测量：</strong>{parseFloat(currentProject.detectionResult.output.scuffingRate).toFixed(1)}% 
                                    {parseFloat(currentProject.detectionResult.output.scuffingRate) > 80 
                                        ? "（大面积损伤 - 请检查干涉情况）" 
                                        : parseFloat(currentProject.detectionResult.output.scuffingRate) > 60 
                                            ? "（中度损伤）" 
                                            : "（轻微损伤）"}
                                </p>
                                <p><strong>点蚀分布情况：</strong>{parseFloat(currentProject.detectionResult.output.pittingRate).toFixed(1)}% 
                                    {parseFloat(currentProject.detectionResult.output.pittingRate) > 60 
                                        ? "（广泛点蚀）" 
                                        : parseFloat(currentProject.detectionResult.output.pittingRate) > 30 
                                            ? "（局部点蚀）" 
                                            : "（初期点蚀）"}
                                </p>
                                ...
                                <p><strong>识别模型：</strong>{currentProject.detectionResult.input.precision}</p>
                            </div>
                        </Col>
                        <Col span={12}>
                            <div className="card" style={{ height: '100%', padding: '16px' }}>
