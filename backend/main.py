    ...
    try:
        ...
        if precision == 'yolov5s':
            ...
            for file_path in file_paths:
                fatigue_arrays.append(process_detection(file_path))
                move_files_to_data(cache_path, full_path)
            
            fatigue_array = fatigue_arrays[0]
            time.sleep(3)

        # 生成模拟结果
        damage_types = ['齿面磨损', '齿根剥落', '齿面胶合', '齿测点蚀']
        result = {}
        ...
        result['heatmap'] = {
            'name': f"{os.path.splitext(heatmap_filename)[0]}_visualized.jpg",
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            if fatigue_array[0] == 1:
                ...
                f.write(f"? 检测状态: {fatigue_array[0]}\n")
            f.write("\n")
            ...
            # 使用的检测模型信息
            f.write("【检测信息】\n")
            f.write(f"• 使用模型: {precision}\n")
            f.write(f"• 检测时间: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"• 检测图像: {os.path.basename(file_paths[0])}\n")
