...
import subprocess
import matplotlib
matplotlib.use('Agg')  # 使用非交互式后端，避免tkinter多线程问题
...
def visualize_detection(cache_folder):
    ...
    image_files = [f for f in os.listdir(cache_folder) 
                  if f.endswith(('.jpg', '.jpeg', '.png')) and  "blockmap" not in f
    ...
    for img_file in image_files:
        ...
        vis_filename = f"blockmap_{time.strftime('%Y%m%d%H%M%S')}.jpg"
        vis_file = os.path.join(cache_folder, vis_filename)
...
def calculate_pixel_counts(cache_folder):
    ...
    image_files = [f for f in os.listdir(cache_folder) 
                  if f.endswith(('.jpg', '.jpeg', '.png')) and not "blockmap" in f]
...
def process_detection(file_path):
    ...
    # 可视化检测结果
    blockmap_path = visualize_detection(cache_folder)
    ...
    return [1, fatigue_array[0], fatigue_array[1], 
            fatigue_array[2], fatigue_array[3], blockmap_path]
